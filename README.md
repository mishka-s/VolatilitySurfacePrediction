...
2. **Interior Local Hybrid Blend & Spike Mitigation:**
   ...
   * The final interior imputation is a weighted combination ($\alpha$) of both:
     $$\text{IV}_{\text{final}} = (1 - \alpha) \cdot \text{IV}_{\text{linear}} + \alpha \cdot \text{IV}_{\text{cubic}}$$
   
   > **Dynamic Regularization:** The framework tracks cross-sectional anomalies. Under standard market conditions, $\alpha = 0.7$ (`ALPHA_INT`). However, if any observed contract violates the `SPIKE_THRESHOLD = 1.0` boundary, a volatility spike is registered. The blending weight automatically downshifts to $\alpha = 0.6$ (`SPIKE_ALPHA_INT`) to suppress runaway higher-degree polynomial oscillations.

3. **Regularized Asymmetric Wing Extrapolation:**
   For unobserved target strikes out-of-the-money (OTM) or deep in-the-money (ITM) that sit outside the liquid boundary, the model drops higher-degree fits to prevent runaway values. It isolates the outermost active contracts (`WING_POINTS = 2`) to determine a local linear trend line. 
   
   The slope ($s$) is projected outward and regularized via directional damping factors applied to the distance from the outermost boundary anchor:
   * **Left Wing Boundaries:** Damped using `LO_WING_DAMP = 0.90`.
   * **Right Wing Boundaries:** Maintained at `HI_WING_DAMP = 1.00`.
   * **Spike Override:** If a volatility spike is identified cross-sectionally, the regular damping rules are overridden, enforcing a uniform factor of `SPIKE_WING_DAMP = 1.0` across both wings.
...
* **`reconstruct_side`:** Executes the core mathematical logic for a single side of the contract chain at a unique timestamp (coordinate scaling, conditional linear/cubic blending under spike parameters, regularized wing extrapolation).
* **`apply_imputer`:** Manages row-by-row matrix processing, applies causal temporal forward fill (`ffill`) fallbacks for intervals where entire cross-sections are missing, handles mean-fill residuals, and handles hard boundary clipping (`[IV_FLOOR, IV_CEILING]`).
...
| :--- | :--- | :--- | :--- |
| `DATA_PATH` | `str` | `"/content/sample_data/dataset.csv"` | Local target filepath containing incoming options data structures. |
...
| `ALPHA_INT` | `float` | `0.7` | Weight assigned to the local polynomial curve vs. the linear baseline spline under normal conditions. |
| `SPIKE_THRESHOLD` | `float` | `1.0` | Empirical IV ceiling parameter used to identify structural high-volatility data drops. |
| `SPIKE_ALPHA_INT` | `float` | `0.6` | Polynomial blending weight deployed when an intraday volatility spike is flagged. |
| `WING_POINTS` | `int` | `2` | Number of terminal liquid contracts used to calculate outbound extrapolation trends. |
| `LO_WING_DAMP` | `float` | `0.90` | Damping multiplier applied to the left-wing extrapolation slope. |
| `HI_WING_DAMP` | `float` | `1.00` | Damping multiplier applied to the right-wing extrapolation slope. |
| `SPIKE_WING_DAMP` | `float` | `1.0` | Overriding wing damping factor applied if a spike condition is actively identified. |
| `MIN_POINTS_PER_SIDE`| `int` | `2` | Minimum active observations required per layout row to attempt cross-sectional calculation. |
| `IV_FLOOR` / `IV_CEILING` | `float` | `0.0` / `8.0` | Physical clipping bounds to ensure outputs remain realistic. |
| `SEP` | `str` | `"\|\|"` | Identifier token string utilized to form flattened target map keys. |
| `EXPIRY_TS` | `Timestamp` | `"2026-01-27 15:30:00"` | Precise cutoff boundary used to compute remaining lifespan metrics ($\tau$). |
| `EXPIRY_DATE` | `date` | `2026-01-27` | Filter criteria targeting operational row boundaries for Phase 2 overriding. |
...
* **`filled_dataset.csv`**: A dense matrix matching the original dataframe shape where all missing elements are fully populated and string representations of time are unified under the `%d-%m-%Y %H:%M` convention.
* **`submission.csv`**: A flattened key-value pairing designed for immediate submission or downstream calculation engines, pairing unique identifiers with their respective optimized implied volatility predictions:
